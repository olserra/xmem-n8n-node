        ...
        for (let i = 0; i < items.length; i++) {
            ...
            if (resource === 'sessionMemory') {
                ...
                if (operation === 'add') {
                    ...
                    const role = this.getNodeParameter('role', i, 'user');
                    responseData = await axios_1.default.post(`${baseUrl}/session-memory/`, { sessionId, content, role }, {
                        headers: {
                    ...
                    responseData = await axios_1.default.get(url, {
                        headers: {
                            'Authorization': `Bearer ${apiKey}`,
                        },
                    });
                ...
                if (operation === 'createUpdate') {
                    ...
                    responseData = await axios_1.default.post(`${baseUrl}/sessions/`, { sessionId, metadata: metadataObj }, {
                        headers: {
                            'Authorization': `Bearer ${apiKey}`,
                            'Content-Type': 'application/json',
                        },
                    ...
                    const sessionId = this.getNodeParameter('sessionId', i, '');
                    responseData = await axios_1.default.get(`${baseUrl}/sessions/${sessionId}`, {
                        headers: {
                            'Authorization': `Bearer ${apiKey}`,
                        },
                    });
                    ...
                    responseData = await axios_1.default.get(`${baseUrl}/sessions/`, {
                        headers: {
                            'Authorization': `Bearer ${apiKey}`,
                        },
                    });
                ...
                if (operation === 'query') {
                    ...
                    const sessionId = this.getNodeParameter('sessionId', i, '');
                    responseData = await axios_1.default.post(`${baseUrl}/agent-chat/`, { input, sessionId }, {
                        headers: {
                            'Authorization': `Bearer ${apiKey}`,
                            'Content-Type': 'application/json',
                        },
                ...
                const operation = this.getNodeParameter('operation', i);
                if (operation === 'list') {
                    responseData = await axios_1.default.get(`${baseUrl}/vector-sources/`, {
                        headers: {
                            'Authorization': `Bearer ${apiKey}`,
                        },
                    });
                    ...
                    responseData = await axios_1.default.post(`${baseUrl}/vector-sources/`, { name, description }, {
                        headers: {
                            'Authorization': `Bearer ${apiKey}`,
                            'Content-Type': 'application/json',
                        },
                ...
                if (operation === 'query') {
                    ...
                    responseData = await axios_1.default.get(`${baseUrl}/qdrant-queries/?query=${encodeURIComponent(query)}`, {
                        headers: {
                            'Authorization': `Bearer ${apiKey}`,
                        },
                    });
                ...
                if (operation === 'create') {
                    const name = this.getNodeParameter('name', i);
                    responseData = await axios_1.default.post(`${baseUrl}/api-keys/`, { name }, {
                        headers: {
                            'Authorization': `Bearer ${apiKey}`,
                            'Content-Type': 'application/json',
                        },
